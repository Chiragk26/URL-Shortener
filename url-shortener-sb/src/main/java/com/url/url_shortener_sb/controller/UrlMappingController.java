...
public class UrlMappingController {
    ...
    //{"originalUrl": "http//../"}
    //principal auto-injected by spring security
    //@PreAuthorize--> to check authorization before a method is executed
@PostMapping("/shorten")
@PreAuthorize("hasRole('USER')")
    public ResponseEntity<UrlMappingDTO> createShortUrl(@RequestBody Map<String,String> request, Principal principal){

    String originalUrl = request.get("originalUrl");
    User user=userService.findByUsername(principal.getName()); //every short url in db is mapped to a user, we user_id in urlMapping db
