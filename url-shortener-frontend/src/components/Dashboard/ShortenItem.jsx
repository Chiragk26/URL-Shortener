...
import ClipLoader from 'react-spinners/ClipLoader'; // ✅ replaced Hourglass with ClipLoader
  ...
  const fetchMyShortUrl = async () => {
    ...
    try {
      const { data } = await api.get(
        `/api/urls/analytics/${selectedUrl}?startDate=2025-01-01T00:00:00&endDate=2027-12-31T23:59:59`,
        {
          headers: {
    ...
    <div className="bg-slate-100 shadow-lg border border-dotted border-slate-500 px-6 sm:py-1 py-3 rounded-md transition-all duration-100">
      <div className="flex sm:flex-row flex-col sm:justify-between w-full sm:gap-0 gap-5 py-5">
        ...
        <div className="flex flex-1 sm:justify-end items-center gap-4">
          <div
            onClick={() => {
              clipboard.copy(import.meta.env.VITE_REACT_FRONT_END_URL + "/s/" + shortUrl);
              setIsCopied(true);
          ...
          {loader ? (
            <div className="min-h-[calc(450px-140px)] flex justify-center items-center w-full">
              <div className="flex flex-col items-center gap-1">
                <ClipLoader color="#306cce" size={50} />
                <p className="text-slate-700">Preparing analytics...</p>
              </div>
            </div>
          ) : (
            <>
